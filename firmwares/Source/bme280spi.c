...
#define LCD_SPI_BEGIN2()    HAL_IO_SET(HAL_LCD_CS2_PORT,  HAL_LCD_CS2_PIN,  0); /* chip select 2*/
#define LCD_SPI_END()                                                         \
{                                                                             \
  asm("NOP");                                                                 \
  asm("NOP");                                                                 \
...
{                                                                             \
  ...
  asm("NOP");                                                                 \
  asm("NOP");                                                                 \
  HAL_IO_SET(HAL_LCD_CS2_PORT,  HAL_LCD_CS2_PIN,  1); /* chip select 2*/         \
}
...
void HalLcd_HW_Init(void){
  /* Perform reset */
  LCD_ACTIVATE_RESET();
  bme_HW_WaitUs(15000); // 15 ms
  LCD_RELEASE_RESET();
  bme_HW_WaitUs(15000); // 15 us
}
...
/*  
  ...
  bme_HW_WaitUs(15000); // 15 ms
  LCD_RELEASE_RESET();
  bme_HW_WaitUs(15); // 15 us
  //LCD Data/Command test
  LCD_DO_WRITE();
  bme_HW_WaitUs(15000); // 15 ms
  LCD_DO_CONTROL();
  bme_HW_WaitUs(15000); // 15 ms
...
void bme_ConfigIO(void)
{
  ...
  HAL_CONFIG_IO_OUTPUT(HAL_LCD_CS2_PORT,   HAL_LCD_CS2_PIN,   1);
...
void bme_ConfigIOInput(void)
{
  ...
  HAL_CONFIG_IO_INPUT(HAL_LCD_CS2_PORT,   HAL_LCD_CS2_PIN,   0);
}
